function count = find_circles(domino)
    ...
    for k = 1 : numberOfBlobs % Loop through all blobs.
        ...
        if blobMeasurements(k).Area < 0.04 * size(image, 1) * size(image, 2) && ...
                blobMeasurements(k).Area > 0.0015 * size(image, 1) * size(image, 2) &&...
                axis_aspect_ratio > 0.45
    ...
    for k = 1 : numberOfBlobs % Loop through all blobs.
        ...
        y = [y1, y1, y2, y2, y1];
        axis_aspect_ratio = blobMeasurements(k).MinorAxisLength / blobMeasurements(k).MajorAxisLength;
        if blobMeasurements(k).Area < 0.05 * size(image, 1) * size(image, 2) && ...
                blobMeasurements(k).Area > 0.0015 * size(image, 1) * size(image, 2) &&...
                axis_aspect_ratio > 0.45
