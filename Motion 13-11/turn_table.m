...
radOffset = atan((turnTable(2) - centY)/(turnTable(1) - centX));
%turnTable = [-198.7948, 203.1205, 91.6687];
moveArm(3, 60000, 200, 200, port);
moveArm(3, 34, 200, 200, port);
...
start = tic;
c = clock;
currentTime = c(5)*60 + c(6)
rads = 50;
range = 0.1;
mod(rads, 2*pi+radOffset)
while mod(rads, 2*pi+radOffset) > range
    %time = toc(start) + (initialTime - currentTime)
    c = clock;
    currentTime = c(5)*60 + c(6)
    time = initialTime - currentTime + 0.5
    rads = speed*time
    mod(rads, 2*pi+radOffset)
end
moveArm(3, 5, 40, 50, port);
